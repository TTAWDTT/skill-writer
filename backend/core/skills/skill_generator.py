"""
Skill Generator - 使用 LLM 和 skill generator 资源来生成新的 Skill
"""
import json
import re
...
from backend.core.skills.validator import validate_generated_skill_config
...
# writer_skill_creator 不可用时的后备系统提示词
FALLBACK_SKILL_CREATOR_SYSTEM_PROMPT = """你是一个专门分析文书模板并创建相应写作 Skill 的专家。你的任务是根据用户提供的文书模板，生成完整的 Skill 配置。
...
def _resolve_skill_creator_system_prompt() -> str:
    """
    优先从 writer_skill_creator skill 中读取生成规范。
    若未找到，再回退到内置常量。
    """
    registry = get_registry()
    generator_skill = registry.get("writer-skill-creator") or registry.get("writer_skill_creator")
    if generator_skill:
        prompt = ""
        try:
            prompt = getattr(generator_skill, "system_prompt", "") or ""
        except Exception:
            prompt = ""
        ...
        try:
            guidelines = generator_skill.writing_guidelines or ""
        except Exception:
            guidelines = ""
        if isinstance(guidelines, str) and guidelines.strip():
            return guidelines.strip()

    return FALLBACK_SKILL_CREATOR_SYSTEM_PROMPT
...
"""
    ...
    messages = [
        {"role": "system", "content": _resolve_skill_creator_system_prompt()},
        {"role": "user", "content": user_prompt}
    ]

    # 调用 LLM
    gateway = get_global_gateway()
    response = await gateway.chat(messages, temperature=0.3, max_tokens=8192)
    ...
    validate_generated_skill_config(skill_config)
